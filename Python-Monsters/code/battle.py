...
class Battle:
	...
	def create_monster(self, monster, index, pos_index, entity):
		...
		if entity == 'player':
			...
			frames = {
				state: [pygame.transform.flip(frame, True, False) for frame in frames] for state, frames in frames.items()
			}
			outline_frames = {
				state: [pygame.transform.flip(frame, True, False) for frame in frames] for state, frames in outline_frames.items()
			}
	...
	def apply_attack(self, target_sprite, attack, amount):
		...
		if attack_element == 'plant' and target_element == 'fire' or \
		   attack_element == 'fire' and target_element == 'water' or \
		   attack_element == 'water' and target_element == 'plant':
			amount *= 0.5

		target_defense = 1 - target_sprite.monster.get_stat('defense') / 1800
			...
			if keys[pygame.K_SPACE]:
				...
				if self.selection_mode == 'target':
					...
					if self.selected_attack:
						...
						if monster_sprite.monster.health < monster_sprite.monster.get_stat('max_health') * 0.4:
							self.monster_data['player'][len(self.monster_data['player'])] = monster_sprite.monster
							monster_sprite.delayed_kill(None)
							self.update_all_monsters('resume')
						else:
							TimedSprite(monster_sprite.rect.center, self.monster_frames['ui']['cross'], self.battle_sprites, 500)
