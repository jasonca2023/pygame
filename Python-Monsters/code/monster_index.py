...
class MonsterIndex:
	...
	def display_main(self, dt):
		...
		element_surf = self.fonts['regular'].render(f'Element: {monster.element}', False, COLORS['white'])
		...
		bar_data = {
			...
			'right_side': rect.left + rect.width * 0.75
		}
		...
		stats_rect = pygame.FRect(sides['left'], healthbar_rect.bottom, healthbar_rect.width, info_height).inflate(0, -60).move(0, 10)
		...
		for index, (stat, value) in enumerate(monster_stats.items()):
			...
			text_rect = text_surf.get_frect(topleft = icon_rect.topleft + vector(30, -7))
			self.display_surface.blit(text_surf, text_rect)
