...
class Game:
    def __init__(self):
        ...
        self.audio['overworld'].set_volume(5)
        self.audio['battle'].set_volume(5)
    ...
    def create_dialogue(self, character):
        if not self.dialogue_tree:
            self.dialogue_tree = DialogueTree(character, self.player, self.all_sprites, self.fonts['dialogue'], self.end_dialogue)
    ...
    def monster_encounter(self):
        ...
        if sprites and self.player.direction:
            ...
            self.audio['overworld'].stop()
            self.audio['battle'].play(-1)
            self.transition_target = self.transition_target = Battle(
                player_monsters = self.player_monsters, 
                opponent_monsters = {index: Monster(monster, sprites[0].level + randint(-5, 5)) for index, monster in enumerate(sprites[0].monsters)}, 
                monster_frames = self.monster_frames, 
    ...
    def run(self):
        while True:
            ...
            if self.dialogue_tree: self.dialogue_tree.update()
